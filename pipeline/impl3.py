...
# add to imports
from models.marine_processor import MarineLogitsProcessor
...
class Pipeline3:
    ...
    def generate(
        ...
        # ── Step 1: get guidance (V, V_neg) ────────────────────────────────
        image_key = self._get_image_key(image, image_path)

        # ── Step 2: prepare original inputs ────────────────────────────────
        messages = [
            {
                "role": "user",
                "content": [
        ...
        if detected_objects:
            # build guidance prompt inputs
            guidance_inputs = self._build_guidance_prompt(
                detected_objects, image, text
            )

            logits_processor = MarineLogitsProcessor(
                model=self.model,
                guidance_input_ids=guidance_inputs["input_ids"],
                guidance_attention_mask=guidance_inputs["attention_mask"],
                gamma=GAMMA,
                alpha=self.model.lm_head.alpha
            )
